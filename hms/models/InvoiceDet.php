...
class InvoiceDet extends CActiveRecord {
    ...
    public function saveCityLedger($data) {
        $siteConfig = SiteConfig::model()->findByPk(1);
        //save invoice
        foreach ($data as $val) {
            $invoiceDet = new InvoiceDet;
            $invoiceDet->description = 'Na pada tanggal ' . $val->date;
            $invoiceDet->user_id = $val->user_id;
//            $invoiceDet->payment = $val->payment;
            $invoiceDet->type = 'customer';
            $invoiceDet->code = 'Cust' . date('Ymdhis');
            $invoiceDet->save();

            $coaDet = new AccCoaDet;
            $coaDet->acc_coa_id = $siteConfig->acc_city_ledger_id;
            $coaDet->invoice_det_id = $invoiceDet->id;
            $coaDet->date_coa = $val->date;
            $coaDet->code = 'Na pada tanggal' . date("m-d-y", strtotime($val->date));
            $coaDet->description = $invoiceDet->description;
            $coaDet->debet = $val->payment;
            $coaDet->credit = 0;
            $coaDet->reff_type = 'invoice';
//            $coaDet->reff_id = $invoiceDet->id;
            $coaDet->save();
        }
