...
class AccCoaDet extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('acc_coa_id, created_user_id, reff_id', 'numerical', 'integerOnly' => true),
...
//            array('description', 'length', 'max' => 255),
            ...
            array('id, acc_coa_id, date_coa, description, debet, credit, balance, created, created_user_id, modified, reff_type, reff_id', 'safe', 'on' => 'search'),
        );
    }
...
//    public function updateAfter($param) {
////        logs($param);
//        foreach ($param as $val) {
//            $balance = $val->balance;
//            $cekModel = $this->findAll(array('condition' => 'reff_type<>"balance" AND date_coa >= "' . $val->date_trans . '" and acc_coa_id="' . $val->acc_coa_id . '"', 'order' => 'date_coa ASC, id ASC'));
//            foreach ($cekModel as $model) {
//                $model->balance = $balance + ($model->debet - $model->credit);
//                $balance = $model->balance;
//                $model->save();
//            }
//        }
//    }
//    public function updateAfterSubLedger($param) {
//        foreach ($param as $val) {
//            $ar = 0;
//            $as = 0;
//            $ap = 0;
...
//            
//            $balance = $val->balance;
//            $cekModel = AccCoaSub::model()->findAll(array('condition' => 'date_coa > "' . $val->date_trans . '" and (ar_id=' . $ar . ' OR as_id=' . $as . ' OR ap_id=' . $ap . ')', 'order' => 'date_coa ASC, id ASC'));
//            foreach ($cekModel as $model) {
//                $model->balance = $balance + ($model->debet - $model->credit);
//                $balance = $model->balance;
//                $model->save();
